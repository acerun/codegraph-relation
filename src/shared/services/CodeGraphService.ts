...
export class CodeGraphService {
    ...
    public async getRelationItems(item: vscode.CallHierarchyItem, direction: 'incoming' | 'outgoing', limit = 100): Promise<vscode.CallHierarchyItem[]> {
        const root = this.findProjectRoot();
        if (!item.name.trim() || !root) {
            this.log(`getRelationItems skipped. symbol="${item.name}", root=${root || '<missing>'}`);
            return [];
        }
